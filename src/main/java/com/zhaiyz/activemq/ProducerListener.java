...
public class ProducerListener implements MessageListener {
	...
	public String convertAndSendMessage(final String message) {
		ReplyMessage replyMessage = new ReplyMessage();
		final String correlationID = UUID.randomUUID().toString();
		concurrentMap.put(correlationID, replyMessage);

		jmsTemplate.convertAndSend(requestDestination, message, new MessagePostProcessor() {
	...
	public void onMessage(Message message) {
		...
		if (message instanceof ObjectMessage) {
			try {
				String msg = (String) (((ObjectMessage) message).getObject());
				concurrentMap.get(message.getJMSCorrelationID()).setMessage(message);
				concurrentMap.get(message.getJMSCorrelationID()).getSemaphore().release();
